...
def estruturar_com_groq(markdown_content, schema_content, api_key):
    ...
    prompt_sistema = (
        ...
        "3. Mapeie todas as seções obrigatórias (titulo, data, local, participantes, resumo_assuntos, encaminhamentos, duvidas_incertezas, texto_corrido).\n"
        "4. Na lista de encaminhamentos, preencha acao, responsavel e prazo de cada item.\n"
        "5. O campo 'texto_corrido' DEVE ser redigido seguindo estritamente as regras de ata legal e burocrática:\n"
        "   - Preâmbulo Burocrático Fiel: O texto deve iniciar descrevendo a data, o horário, o local e o nome de todos os participantes presentes. Use ESTRITAMENTE as informações de data, horário, local e participantes que constam no Markdown de entrada. NUNCA utilize dados do exemplo a seguir. A data, o ano e o horário devem ser escritos inteiramente por extenso. Exemplo de estrutura de início: 'Aos vinte e nove dias do mês de maio de dois mil e vinte e seis, às dez horas, na sala do ESC da Universidade Estadual do Tocantins, reuniram-se...' seguido da lista de participantes separados por vírgula.\n"
        "   - Riqueza de Detalhes (Não Simplificar): O texto corrido deve ser altamente descritivo e completo, narrando minuciosamente e em formato de texto discursivo cada um dos assuntos discutidos, encaminhamentos tomados (com responsáveis e prazos) e dúvidas registradas no Markdown de origem. NÃO resuma de forma rasa; o texto deve abranger todos os detalhes originais das discussões de forma fluida.\n"
        "   - Texto Contínuo com Pontuação Adequada: O texto corre do início ao fim sem qualquer quebra de linha (sem '\\n'), sem marcadores de tópicos (como hifens ou asteriscos no início de linhas) e sem recuos ou parágrafos. Tudo é um único e imenso bloco. UTILIZE PONTOS FINAIS INTERNOS normalmente para concluir pensamentos e separar assuntos ao longo do texto corrido, facilitando a legibilidade.\n"
